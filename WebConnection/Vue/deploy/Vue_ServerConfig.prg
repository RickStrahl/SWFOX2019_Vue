...
***            Vue.ini and you can customize operation there.
***
***            [ServerConfig]
***            Virtual=Vue            
***            ScriptMaps=wc,wcs,wcsx,wcvue    
...
***
***    Assume: Called from VueMain.prg startup code
***            with lcAction parameter:
***            DO VueMain with "CONFIG"
***
***            Or launch your Exe from the Windows Command Line:
***            Vue.exe config
***
***            or with a specific IIS site/virtual
***            Vue.exe config "IIS://localhost/w3svc/12/root"
***
...
IF FILE("WCONNECT.APP")
   MessageBox("This feature only works in the full version of Web Connection",48,"Vue")
   RETURN
ENDIF
...
IF !IsAdmin() 
   MESSAGEBOX("Admin privileges are required to configure the IIS Web Server." + CHR(13) +;
              ...
              48,"Vue Server Configuration")
   RETURN
ENDIF

*** Try to read from Vue.ini [ServerConfig] section
loApi = CREATEOBJECT("wwAPI")
lcIniPath = FULLPATH("Vue.ini")
...
IF ISNULLOREMPTY(lcScriptMaps)
  lcScriptMaps = "wc,wcs,wcsx,wcvue"  
ENDIF
IF EMPTY(lcIISPath)
...
IF FILE("Vue.exe")
   RUN /n4 "Vue.exe" /regserver

   *** Optionally set DCOM permission - only set if needed
   *** requires that DComLaunchPermissions.exe is available in .\tools\
   * DCOMLaunchPermissions("Vue.VueServer","INTERACTIVE")
   * DCOMLaunchPermissions("Vue.VueServer","username","password")
ENDIF
